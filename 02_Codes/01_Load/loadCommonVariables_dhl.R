...
suppressMessages({
  ...
  commonVarialbes <- tryCatch({
    ...
    carryingFeeOver20 <<- commonVarialbes$value[6]
    CODRate <<- commonVarialbes$value[7]
    CODThreshold <<- commonVarialbes$upper_bound[7]
    insuranceFeeRate <<- commonVarialbes$value[8]
    insuranceFeeThreshold <<- commonVarialbes$upper_bound[8]
